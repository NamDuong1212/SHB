            ...
            <div class="col-span-12 lg:col-span-9 mt-4">
                <div class="flex justify-between w-full mb-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-xl font-bold text-primary m-0">PHÊ DUYỆT ĐIỀU CHUYỂN</h1>
                    </div>
                                ...
                                <div>
                                    <div class="border rounded-lg p-3">
                                        ...
                                        <div class="p-float-label mb-3">
                                            ...
                                            <TreeSelect id="managingUnit" v-model="form.managingUnitId" :options="optionsOrg" size="small" optionLabel="label" class="w-full" placeholder="Chọn bộ phận" disabled />
                                        </div>
                                    ...
                                    <div class="border rounded-lg p-3">
                                        ...
                                        <div class="p-float-label mb-3">
                                            ...
                                            <TreeSelect id="toManagingUnit" v-model="form.toManagingUnitId" :options="optionsOrg" size="small" optionLabel="label" class="w-full" placeholder="Chọn bộ phận" disabled />
                                        </div>

                                        <div class="p-float-label">
                                            <label for="toLocation">Vị trí</label>
                                            <Dropdown id="toLocation" size="small" v-model="form.toLocationId" :options="optionLocation" optionLabel="factoryName" optionValue="id" class="w-full mt-1" disabled />
                                        </div>
                                    </div>
                                </div>
...
const searchAsset = async (event) => {
    try {
        const result = await DeviceTransferService.searchDevice(event.query);
...
const loadLocations = async (fromUnitId, toUnitId) => {
    try {
        if (fromUnitId) {
            const resFrom = await DeviceTransferService.getLocation(fromUnitId);
            optionLocation.value = resFrom.data;
        }

        if (toUnitId) {
            const resTo = await DeviceTransferService.getLocation(toUnitId);
...
const onSubmit = async () => {
    try {
        ...
        await DeviceTransferService.approve(data);
