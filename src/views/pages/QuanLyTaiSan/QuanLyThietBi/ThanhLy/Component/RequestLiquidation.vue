            ...
            <div class="col-span-12 lg:col-span-9 mt-4">
                <div class="flex justify-between w-full mb-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-xl font-bold text-primary m-0">YÊU CẦU THANH LÝ</h1>
                    </div>
                ...
                <DataTable :value="form.deviceLines" class="p-datatable-sm" :rowHover="true" showGridlines stripedRows responsiveLayout="scroll">
                    ...
                    <Column field="assetLiquidationPrice" header="Giá thanh lý">
                        <template #body="{ data, index }">
                            <div class="p-input-filled">
                                <InputNumber
                                    size="small"
                                    v-model="data.assetLiquidationPrice"
                                    class="w-full"
                                    placeholder="Vui lòng nhập giá thanh lý"
                                    :invalid="messages[`deviceLines[${index}].assetLiquidationPrice`]"
                                    mode="currency"
                                    currency="VND"
                                    locale="vi-VN"
                                />
                                <small class="text-red-500" v-if="messages[`deviceLines[${index}].assetLiquidationPrice`]">
                                    {{ messages[`deviceLines[${index}].assetLiquidationPrice`] }}
                                </small>
                            </div>
                        </template>
                    </Column>
                    <Column field="assetLiquidationFee" header="Phí thanh lý">
                        <template #body="{ data, index }">
                            <div class="p-input-filled">
                                <InputNumber
                                    size="small"
                                    v-model="data.assetLiquidationFee"
                                    class="w-full"
                                    placeholder="Vui lòng nhập phí thanh lý"
                                    :invalid="messages[`deviceLines[${index}].assetLiquidationFee`]"
                                    mode="currency"
                                    currency="VND"
                                    locale="vi-VN"
                                />
                                <small class="text-red-500" v-if="messages[`deviceLines[${index}].assetLiquidationFee`]">
                                    {{ messages[`deviceLines[${index}].assetLiquidationFee`] }}
                                </small>
                            </div>
                        </template>
                    </Column>
...
const searchAsset = async (event) => {
    try {
        ...
        const result = await DeviceLiquidationService.searchAsset(event.query, keyLocation, keyManagin);
...
const handleUpdateRequest = (event) => {
    if (event.value) {
        ...
        form.value.deviceLines.push({
            ...
            assetLiquidationPrice: null,
            assetLiquidationFee: null
        });
        setTimeout(() => {
            keySearch.value = null;
        }, 10);
...
const handleChangeManagingUnit = async (event) => {
    ...
    if (form.value.managingUnitId) {
        try {
            const res = await DeviceLiquidationService.getLocation(Object.keys(event)[0]);
...
const onSubmit = async () => {
    try {
        ...
        await DeviceLiquidationService.save(data);
