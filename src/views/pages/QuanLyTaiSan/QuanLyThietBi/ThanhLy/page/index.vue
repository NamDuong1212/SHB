        ...
        items: [
            {
                ...
                icon: 'pi pi-file',
                command: () => {
                    router.push('/thanh-ly-thiet-bi/yeu-cau');
                }
            },
            {
                label: 'Phê duyệt',
                icon: 'pi pi-verified',
                command: async () => {
                    router.push('/thanh-ly-thiet-bi/phe-duyet');
                }
            }
        ]
    }
...
<template>
    <tableDoc ref="tableRef" header="Danh sách thanh lý thiết bị" :columns="columns" :filters="filters" apiEndpoint="DeviceLiquidation" @resetFilter="initFilters" v-model:expandedRows="expandedRows" :isExpanded="true">
        <template #header>
            <Button size="small" type="button" icon="pi pi-chevron-down" iconPos="right" label="Thanh lý" @click="toggle" aria-haspopup="true" aria-controls="overlay_menu" />
