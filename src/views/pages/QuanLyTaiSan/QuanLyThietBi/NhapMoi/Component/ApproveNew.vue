            ...
            <div class="col-span-12 lg:col-span-9 mt-4">
                <div class="flex justify-between w-full mb-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-xl font-bold text-primary m-0">PHÊ DUYỆT PHIẾU NHẬP</h1>
                    </div>
                ...
                <DataTable :value="form.deviceLines" class="p-datatable-sm" :rowHover="true" showGridlines stripedRows responsiveLayout="scroll">
                    ...
                    <Column field="assetValue" header="Giá trị tài sản">
                        <template #body="{ data }">
                            <div class="text-gray-700">{{ format.vnd(data.assetValue) }}</div>
                        </template>
                    </Column>
                </DataTable>
            </div>
                        ...
                        <template #content>
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="p-float-label">
                                    ...
                                    <InputText size="small" id="decisionNumber" v-model="form.decisionNumber" class="w-full mt-1" readonly />
                                ...
                                <div class="p-float-label">
                                    ...
                                    <Calendar readonly size="small" id="decisionDate" v-model="form.decisionDate" dateFormat="yy-mm-dd" class="w-full mt-1" :showIcon="true" />
...
const dataOrganizationalStructure = ref({});
...
const searchAsset = async (event) => {
    try {
        const result = await DeviceAdditionService.searchDevice(event.query);
...
const handleUpdateRequest = async (event = null, data = null) => {
    ...
    if (val) {
        try {
            ...
            form.value = { ...val, managingUnitId: { [val.managingUnitId]: true } };
            keySearch.value = val.assetCode;

            // Get organization structure
            const resOrg = await DeviceAdditionService.getByOrganizationalStructure(val.managingUnitId);
            dataOrganizationalStructure.value = resOrg.data;

            // Get location options
            await handleChangeManagingUnit(val.managingUnitId);
        } catch (error) {
...
const handleChangeManagingUnit = async (id) => {
    optionLocation.value = [];
    if (id) {
        try {
            const res = await DeviceAdditionService.getLocation(id);
...
const onSubmit = async () => {
    try {
        if (form.value.managingUnitId) {
            const id = Object.keys(form.value.managingUnitId);
            form.value.managingUnitId = parseInt(id);
        }
        await DeviceAdditionService.approve(form.value);
        proxy.$notify('S', 'Yêu cầu nhập mới tài sản đã được phê duyệt thành công', toast);
...
const onReset = () => {
    ...
    optionSearch.value = [];
    dataOrganizationalStructure.value = {};
    messages.value = {};
    keySearch.value = null;
};
</script>
